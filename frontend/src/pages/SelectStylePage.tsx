...
// import generateMusic from '../pages/PictureUploadPage';
function SelectStylePage() {
  ...
  //추가-localstorage를 쓸 경우
  const [, setTaskId] = useState<string | null>(null);
  const [, setUserId] = useState<string | null>(null); // user_id 상태 추가
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          alert(`아로고 생성 도중 오류가 발생했습니다: ${errorMessage}`);
        } else {
          alert('뿅로 생성 도중 오류가 발생했습니다.');
        }
        console.error('There was an error!', error);
      } else {
        alert('고로 생성 도중 예기치 않은 오류가 발생했습니다.');
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          const errorMessage =
            error.response.data.detail || JSON.stringify(error.response.data);
          console.log(JSON.stringify(error.response.data));
          alert(`1음악 생성 도중 오류가 발생했습니다1: ${errorMessage}`);
        } else {
          alert('음악 생성 도중 오류가 발생했습니다.2');
        }
        console.error('There was an error!', error);
      } else {
        alert('음악 생성 도중 예기치 않은 오류가 발생했습니다.3');
        console.error('There was an unexpected error!', error);
      }
      //navigate('/pictureupload');
    }
        ...
        <NoNoteBackground>
          ...
          <div className="relative flex h-[30rem] w-[80rem] flex-col items-center rounded-[40px] border-2 border-white shadow">
            ...
            <div className="mt-28 flex flex-col items-center justify-center gap-16">
              ...
              <div className="mt-16 flex w-full flex-row justify-between px-4">
                <button
